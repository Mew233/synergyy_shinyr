...
server <- function(input, output,session){
    ...
    output$columns1 <- renderUI({
        selectInput("d1", label = "Drug1", choices = c('All',sort(unique(as.character(data()$Drug1)))))
    })
    output$columns2 <- renderUI({
        selectInput("d2", label = "Drug2", choices = c('All',sort(unique(as.character(data()$Drug2)))))
    })
    output$columns3 <- renderUI({
        selectInput("cell", label = "Cell line", choices = c('All',sort(unique(as.character(data()$DepMap_ID)))))
    })
    ...
    output$heatImage <- renderPlot({
        ...
        frames$Proba = (frames$Proba > 0.5)
        frames$Actuals = frames$Actuals
        frames %>% mutate(Color = ifelse(Proba == Actuals, "green", "red"))
        ...
        if (input$cell == "All") {
            ...
            p +labs(title=input$cell) + theme(axis.text.x=element_text(size=10,angle=45,hjust=1), axis.text.y=element_text(size=6)) +scale_y_discrete(labels = abbreviate) + (
                ...
                (geom_raster(aes(fill = Proba))) +
                geom_rect(data=frames, size=1, fill=NA, colour="green", #需要修改
                          aes(xmin=Var1 - 0.5, xmax=Var1 + 0.5, ymin=Var2 - 0.5, ymax=Var2 + 0.5))  +
                geom_rect(data=frames1, size=1, fill=NA, colour="green",
